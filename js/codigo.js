...
let arrayHoras = ["9:00","9:30","10:00","10:30","11:00","11:30","12:00","12:30","13:00","13:30","14:00","14:30","15:00","15:30","16:00","16:30","17:00","17:30","18:00","18:30","19:00","19:30","20:00","20:30"];
...
function precargarDatos(){
    ...
    if (sistema.listaBarberos.length === 0) {
        ...
        sistema.registrarServicio("Corte de cabello", "Un corte clásico y preciso adaptado a tu estilo. Nuestros barberos combinan técnicas tradicionales con las últimas tendencias para asegurar un acabado impecable. Desde un corte limpio y conservador hasta un estilo moderno y atrevido, saldrás con la confianza de un look perfectamente definido.", 500, 30);
        sistema.registrarServicio("Afeitado clásico", "Experimenta el ritual atemporal del afeitado clásico con toallas calientes. Usamos aceites pre-afeitado y cremas de alta calidad para suavizar la piel y preparar el vello, garantizando un afeitado al ras y sin irritación. Relájate y deja que la navaja afilada de nuestros expertos te deje una piel suave y rejuvenecida.", 350, 30);
        sistema.registrarServicio("Coloración", "¿Buscas un cambio radical o solo un toque de frescura? Nuestro servicio de coloración está diseñado para realzar tu estilo. Desde disimular las canas con un color sutil y natural hasta atreverte con tonos vibrantes, nuestros especialistas te guiarán para elegir el color perfecto que complemente tu personalidad y tono de piel.", 700, 30);
        sistema.registrarServicio("Lavado y acondicionamiento", "Un servicio de cuidado profundo para tu cabello. Olvídate del estrés del día a día con un masaje relajante en el cuero cabelludo mientras lavamos tu cabello con productos premium. Finalizamos con un acondicionamiento que nutre, fortalece y deja tu cabello suave, brillante y con un aroma refrescante.", 400, 30);
        sistema.registrarServicio("Servicio premium con vitaminas", "Eleva tu experiencia de barbería a otro nivel. Este servicio exclusivo incluye un corte de cabello y afeitado de precisión, seguidos de un tratamiento capilar con vitaminas. Esta infusión nutritiva revitaliza el cabello y el cuero cabelludo, promoviendo un crecimiento sano y dejándolo visiblemente más fuerte y saludable. Una verdadera inversión en tu bienestar y apariencia.", 1200, 30);
    }
...
function mostrarSeccionServicios() {

  const seccionServicios = document.querySelector("#seccionServicios .servicios-barberia");

  if (!seccionServicios) return;

  seccionServicios.innerHTML = "<h2>Nuestros Servicios</h2>";

  const listaServicios = sistema.listaServicios;
  
  let html = "";

  for (let i = 0; i < listaServicios.length; i++) {
    const s = listaServicios[i];
    const servicioClase = i % 2 === 0 ? "servicio-item" : "servicio-item servicio-derecha";

    html += `
            <div class="${servicioClase}">
                <div class="servicio-contenido">
                    <h3>${s.nombre}</h3>
                    <p>${s.descripcion}</p>
                    <p class="servicio-datos">
                        <span>Duración: ${s.duracion} min</span> | 
